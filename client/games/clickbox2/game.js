...
function create() {
  ...
  box.on("pointerdown", () => {
    ...
    if (getLevelChecker(currentLevel).succeed({ score })) {
      showDoubleOrFinish(score, () => {
        window.parent.postMessage({ type: "finish", score, level: currentLevel, gameId: GAME_ID }, "*");
      }, () => {
        window.parent.postMessage({ type: "double", score, level: currentLevel, gameId: GAME_ID }, "*");
      }, currentLevel, false);
      incrementLevel();
    }
  });
